...
class GRUCell:
    ...
    def forward(self, x, h_prev=None, c_prev=None, counter=None):
        ...
        cf = c_prev * np.ones(i.shape)-i
        ci = i * c_prev
        c = self.sg(cf+ci)
        h = np.tanh(c_) * o
        self.i_o_c_prev = [(self.i_o_c_prev[0]+i)/2, (self.i_o_c_prev[1] + o)/2, (self.i_o_c_prev[2] + c)/2] if self.i_o_c_prev is not None else [i, o, c]
        if counter is None:
            counter = 1
        else:
            counter += 1
    ...
    def backward(self, grd_O, grd_C, grd_I, grd_F):
        grd_O = self.der_sg(np.tanh(self.i_o_c_prev[2]) @ grd_O) @ grd_O
        grd_C = np.tanh(self.i_o_c_prev[2]) * self.i_o_c_prev[1] * self.der_tanh(self.i_o_c_prev[2])
        grd_I = self.der_tanh(self.i_o_c_prev[2] * grd_C) * grd_C
        grd_C = self.der_sg(grd_C * self.i_o_c_prev[0]) * grd_C
        self.grd_i_o_c = [(self.grd_i_o_c[0] + grd_I)/2 , (self.grd_i_o_c[1] + grd_O)/2, (self.grd_i_o_c[2] +grd_C)/2] if self.grd_i_o_c is not None else [grd_I, grd_O, grd_C]
        return grd_O, grd_C, grd_I

    def step(self):
        self.inputmatrix = self.inputmatrix - self.lr*self.grd_i_o_c[0]
        self.outputmatrix = self.outputmatrix - self.lr*self.grd_i_o_c[1]
        self.weigthmatrix = self.weigthmatrix - self.lr*self.grd_i_o_c[2]
    ###########
    def get_args(self):
        return self.insize, self.outsize, self.lr, self.bias

    def get_params(self):
        return self.inputmatrix, self.outputmatrix, self.weigthmatrix
